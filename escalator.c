...
void mall_control_loop(int simulation_time) {
    ...
    int max_generation_time = mall->current_time + 300; // 最多生成300秒的顾客
    
    while (mall->current_time < end_time) {
        printf("\n========== 时间: %d 秒 ==========\n", mall->current_time);
        ...
        if (mall->current_time < max_generation_time) {
            ...
            if (max_new_customers > 0) {
                // 从-20到max_new_customers之间随机选择一个数
        ...
        // 如果已过300秒且商场空了，可以提前结束
        if (mall->current_time >= max_generation_time && mall->total_customers == 0) {
            printf("超过300秒且商场已空，模拟结束\n");
            break;
        }
